...
static const uint8_t Rcon[Nr] = {
    ...
    0x36  /* x^4 + x^4 + x^2 + x^1 */
};

static void SubWord(uint8_t word[WORD_SIZE])
{
    word[0] = sbox[word[0]];
    word[1] = sbox[word[1]];
    word[2] = sbox[word[2]];
...
static void RotWord(uint8_t word[WORD_SIZE]) 
{
    uint8_t tempRot;
...
static void KeyExpansion128(const uint8_t *key, uint8_t *w) 
{    
    ...
    while (i < Nb * Nk * (Nr + 1)) {
        if (i % (Nk * Nb) == 0) {
            ...
            for (k = 0; k < WORD_SIZE; k++) {
                w[i + k] = w[i - (Nb * Nk) + k] ^ temp[k];

            }

            i += (WORD_SIZE - 1);
        } else {
            w[i] = w[i - Nk] ^ w[i - (Nb * Nk)];
        }
    ...
    uint8_t round)
{
    ...
    for (i = 0; i < Nb; i++) {
        for (j = 0; j < Nb; j++) {
            temp_w[j][i] = w[Nk * (i + round * Nb) + j];
        }
    }

    for (i = 0; i < Nb; i++) {
        for (j = 0; j < Nb; j++) {
            state[i][j] = state[i][j] ^ temp_w[i][j];
        }
    }
}
...
static void SetInput(uint8_t state[][4], const uint8_t *input)
{
    for (uint8_t i = 0; i < Nb; i++) {
        for (uint8_t j = 0; j < Nb; j++) {
...
static void SetOutput(uint8_t state[][4], uint8_t *output)
{
    for (uint8_t i = 0; i < Nb; i++) {
        for (uint8_t j = 0; j < Nb; j++) {
...
void aes128enc(const uint8_t *k, const uint8_t *p, uint8_t *c) 
{
    uint8_t round;
    uint8_t state[4][4];
    uint8_t roundKeys[Nb * Nk * (Nr + 1)];

    KeyExpansion128(k, roundKeys);
